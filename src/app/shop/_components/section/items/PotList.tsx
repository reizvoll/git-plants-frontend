...
const PotList = ({ items, loading }: PotListProps) => {
  ...
  return (
    ...
    >
      <h2 id="pot-list-title" className="text-center text-title2 text-primary-default xs:text-subtitle s:text-title1">
        {t("title")}
      </h2>
        ...
        {loading ? (
          <ul className="flex w-full flex-wrap items-center justify-center gap-6">
            {Array.from({ length: SKELETON_COUNT.MOBILE }).map((_, index) => (
              <ShopItemSkeleton key={index} variant="mobile" itemType="pot" />
            ))}
          </ul>
        ) : items.length > 0 ? (
